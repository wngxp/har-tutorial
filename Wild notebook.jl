...
begin
	using Downloads,ZipFile,DelimitedFiles
...
begin
	base = "/Users/wxp/Documents/Co-op/3Wolong/repos/har-tutorial/HARDataset/UCI HAR Dataset"
	@show isdir(base)
end
...
begin
    function load_inertial_group(split::String; base::String)
        inertial_dir = joinpath(base, split, "Inertial Signals")
        ...
        mats = [readdlm(joinpath(inertial_dir, f), Float32) for f in files]

        N, T = size(mats[1])
        C = length(mats)

        X = Array{Float32}(undef, N, T, C)
        for (i, m) in enumerate(mats)
            X[:, :, i] .= m
        end

        # Convert (N, T, C) → (T, C, N) for Flux Conv1D
        X = permutedims(X, (2, 3, 1))

        y = vec(Int.(readdlm(joinpath(base, split, "y_$(split).txt"))))
        return X, y
    end

    trainX, trainy = load_inertial_group("train"; base=base)
    testX,  testy  = load_inertial_group("test";  base=base)

    size(trainX), size(testX), sort(unique(trainy))
end

# ╔═╡ d7b39562-eb02-4a0a-8eb3-ac0efe203681
begin
    classes = 1:6
    trainY = Flux.onehotbatch(trainy, classes)
    testY  = Flux.onehotbatch(testy, classes)

    accuracy(model, X, Y) = mean(Flux.onecold(model(X), classes) .== Flux.onecold(Y, classes))
end

# ╔═╡ 0122950f-e63b-4b6e-9e75-0ba24a78c9f2
begin
    T, C, _ = size(trainX)          # (128, 9, N)
    n_outputs = length(classes)

    function make_model(; dropout_p=0.5)
        Chain(
            Conv((3,), C => 64, relu),          # kernel size 3
            Conv((3,), 64 => 64, relu),
            Dropout(dropout_p),
            MaxPool((2,)),                      # pool size 2
            Flux.flatten,
            Dense(((T - 2 - 2) ÷ 2) * 64, 100, relu),  # compute flattened size
            Dense(100, n_outputs),
            softmax
        )
    end
...
# ╔═╡ 1b6d5d0e-bbae-4aaf-b9f2-3c61dbe71511
begin
    using Flux: DataLoader

    function evaluate_model(trainX, trainY, testX, testY; epochs=10, batch_size=32, lr=1e-3)
        model = make_model()
        opt = Flux.setup(Adam(lr), model)

        loader = DataLoader((trainX, trainY); batchsize=batch_size, shuffle=true)
...
# ╔═╡ df5cf454-f241-4180-924a-9a4ece077d48
begin
    function run_experiment(repeats=10)
        scores = Float64[]
        for r in 1:repeats
            acc = evaluate_model(trainX, trainY, testX, testY) * 100
            println(">#", r, ": ", round(acc; digits=3))
            push!(scores, acc)
        end
        m = mean(scores); s = std(scores)
        println("Accuracy: $(round(m; digits=3))% (+/-$(round(s; digits=3)))")
        return scores
    end

    scores = run_experiment(10)
end
...
manifest_format = "2.0"
project_hash = "cdde6c9a3eaf322d8b3789701d513ddfd312be38"
